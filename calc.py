...
class Calculator(QMainWindow):
    ...
    def beautiful_number(self):
        try:
            if '.' in self.stroka and not self.flag2 and not self.flag4:
    ...
    def percent(self):
        try:
            if self.itog == '':
                if '.' in self.stroka:
                    self.a = self.stroka.index('.') - 2
                    self.stroka = self.stroka.replace('.', '')
                    self.stroka = (self.stroka[:self.a] + '.' + self.stroka[self.a:])
                    self.itog = self.stroka
                    self.flag2 = True
                    self.beautiful_number()
                else:
                ...
                if '.' in self.itog:
                    self.a = self.itog.index('.') - 2
                    self.itog = self.itog.replace('.', '')
                    self.itog = (self.itog[:self.a] + '.' + self.itog[self.a:])
                    self.stroka = self.itog
                    self.flag2 = True
                    self.beautiful_number()
                else:
                    self.stroka = str(int(self.itog.replace(' ', '')) / 100)
                    self.itog = self.stroka
                    self.flag2 = True
                    print(self.stroka, '||||', self.numbers_probels, '||||', self.itog, '\n')
                    self.beautiful_number()
